...
{ \
    ...
    { \
        ...
        { \
            onReceivePtr(usiTwiDataInReceiveBuffer()); \
        } \
    } \
}
...
static usi_onReceive_t          onReceivePtr;
static usi_onRequest_t          onRequestPtr;

static volatile overflowState_t overflowState;

static uint8_t          rxBuf[ USI_TWI_RX_BUFFER_SIZE ];
static volatile uint8_t rxHead;
static volatile uint8_t rxTail;
static volatile uint8_t rxCount;

static uint8_t          txBuf[ USI_TWI_TX_BUFFER_SIZE ];
...
void usiTwiSlaveInit( uint8_t ownAddress, usi_onReceive_t receiveFunc, usi_onRequest_t requestFunc )
{

  flushTwiBuffers( );

  slaveAddress = ownAddress;
  onReceivePtr = receiveFunc;
  onRequestPtr = requestFunc;
...
void usiTwiTransmitByte( uint8_t data )
{
  ...
  while ( txCount == USI_TWI_TX_BUFFER_SIZE) ;

  // store data in buffer
  txBuf[ txHead ] = data;
  txHead = ( txHead + 1 ) & USI_TWI_TX_BUFFER_MASK;
  txCount++;
...
uint8_t usiTwiReceiveByte( void )
{
  ...
  rxTail = ( rxTail + 1 ) & USI_TWI_RX_BUFFER_MASK;
  rxCount--;
...
ISR( USI_OVERFLOW_VECTOR )
{
  ...
  switch ( overflowState )
  {
    ...
    case USI_SLAVE_SEND_DATA:
      ...
      {
        USIDR = txBuf[ txTail ];
        txTail = ( txTail + 1 ) & USI_TWI_TX_BUFFER_MASK;
        txCount--;
      }
      else
      {
    ...
    case USI_SLAVE_GET_DATA_AND_SEND_ACK:
      ...
      // check buffer size
      if ( rxCount < USI_TWI_RX_BUFFER_SIZE )
      {
        rxBuf[ rxHead ] = USIDR;
        rxHead = ( rxHead + 1 ) & USI_TWI_RX_BUFFER_MASK;
        rxCount++;
      } else {
        // overrun
        // drop data
